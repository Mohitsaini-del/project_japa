  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        toast({
          title: "Welcome Back",
          description: "Successfully signed in. May your practice be peaceful.",
          variant: "success",
        });
        router.push("/dashboard");
        router.refresh();
    ...
    <div className="min-h-screen bg-bg-zen flex items-center justify-center p-4 md:p-6 select-none font-sans">
      <div className="w-full max-w-md bg-white rounded-3xl border border-neutral-100 p-6 md:p-8 shadow-xl shadow-neutral-200/40">
        ...
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          <h2 className="text-xl font-bold text-neutral-800 tracking-tight">Sign in to JapaTrack</h2>
          <p className="text-xs text-neutral-400 font-semibold mt-1">Cultivate stillness in your daily chanting</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="••••••••"
              className={`w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-800 placeholder-neutral-400 focus:border-saffron focus:ring-1 focus:ring-saffron/30 outline-none transition-all duration-200 mt-1.5 ${
            ...
            <button
              ...
              className="absolute right-4 top-10 text-neutral-400 hover:text-neutral-600 transition-colors p-1"
            >
              {showPassword ? <EyeOff className="h-4.5 w-4.5" /> : <Eye className="h-4.5 w-4.5" />}
            </button>
            {errors.password && (
              <span className="text-xs font-medium text-red-505 mt-1 block">
        ...
        <div className="mt-8 text-center">
          <p className="text-xs text-neutral-400 font-medium">
            New to JapaTrack?{" "}
            <Link href="/register" className="text-saffron font-bold hover:underline select-none">
              Sign Up
            </Link>
          </p>
        </div>
